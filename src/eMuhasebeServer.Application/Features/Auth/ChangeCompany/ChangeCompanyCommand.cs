...
public sealed record ChangeCompanyCommand(
    Guid CompanyId) : IRequest<Result<LoginCommandResponse>>;
    ...
    IJwtProvider jwtProvider) : IRequestHandler<ChangeCompanyCommand, Result<LoginCommandResponse>>
{
    public async Task<Result<LoginCommandResponse>> Handle(ChangeCompanyCommand request, CancellationToken cancellationToken)
    {
        ...
        List<Company> companies = companyUsers
        ...
        LoginCommandResponse response = await jwtProvider.CreateToken(appUser, request.CompanyId, companies);
